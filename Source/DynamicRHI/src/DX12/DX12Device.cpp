...
namespace FTS
{
    ...
    BOOL FDX12Device::ResizeBindlessSet(IBindlessSet* pBindlessSet, UINT32 dwNewSize, BOOL bKeepContents)
    {
        ...
        if (dwNewSize <= dwCapacity)
        {
            m_pDescriptorHeaps->ShaderResourceHeap.ReleaseDescriptors(dwOriginalFirstViewIndex + dwNewSize, dwCapacity - dwNewSize);
            pDX12DescriptorTable->m_dwCapacity = dwNewSize;

            return true;
        }

        if (!bKeepContents && dwCapacity > 0)
        {
            m_pDescriptorHeaps->ShaderResourceHeap.ReleaseDescriptors(dwOriginalFirstViewIndex, dwCapacity);
        }

        UINT32 dwNewFirstViewIndex = m_pDescriptorHeaps->ShaderResourceHeap.AllocateDescriptors(dwNewSize);
        pDX12DescriptorTable->m_dwFirstDescriptorIndex = dwNewFirstViewIndex;

        if (bKeepContents && dwCapacity > 0)
        {
            m_Context.pDevice->CopyDescriptorsSimple(
                dwCapacity, 
                m_pDescriptorHeaps->ShaderResourceHeap.GetCpuHandle(dwNewFirstViewIndex), 
                m_pDescriptorHeaps->ShaderResourceHeap.GetCpuHandle(dwOriginalFirstViewIndex), 
    ...
    BOOL FDX12Device::WriteBindlessSet(IBindlessSet* pDescriptorTable, const FBindingSetItem& crBindingItem)
    {
        ...
        case EResourceType::Texture_SRV: 
            {
                FDX12Texture* pDX12Texture = CheckedCast<FDX12Texture*>(crBindingItem.pResource);
                pDX12Texture->CreateSRV(ViewHandle.ptr, crBindingItem.Format, crBindingItem.Dimension, crBindingItem.Subresource);
                break;
            }
        case EResourceType::Texture_UAV:
            {
                FDX12Texture* pDX12Texture = CheckedCast<FDX12Texture*>(crBindingItem.pResource);
                pDX12Texture->CreateUAV(ViewHandle.ptr, crBindingItem.Format, crBindingItem.Dimension, crBindingItem.Subresource);
                break;
            }
        ...
        case EResourceType::RawBuffer_SRV:
            {
                FDX12Buffer* pDX12Buffer = CheckedCast<FDX12Buffer*>(crBindingItem.pResource);
                pDX12Buffer->CreateSRV(ViewHandle.ptr, crBindingItem.Format, crBindingItem.Range, crBindingItem.Type);
                break;
            }
        ...
        case EResourceType::RawBuffer_UAV:
            {
                FDX12Buffer* pDX12Buffer = CheckedCast<FDX12Buffer*>(crBindingItem.pResource);
                pDX12Buffer->CreateUAV(ViewHandle.ptr, crBindingItem.Format, crBindingItem.Range, crBindingItem.Type);
                break;
            }
        case EResourceType::ConstantBuffer:         
            {
                FDX12Buffer* pDX12Buffer = CheckedCast<FDX12Buffer*>(crBindingItem.pResource);
                pDX12Buffer->CreateCBV(ViewHandle.ptr, crBindingItem.Range);
                break;
            }
        ...
        m_pDescriptorHeaps->ShaderResourceHeap.CopyToShaderVisibleHeap(pDX12DescriptorTable->m_dwFirstDescriptorIndex + crBindingItem.dwSlot);
