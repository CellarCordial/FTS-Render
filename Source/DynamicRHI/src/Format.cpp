...
namespace FTS
{
    // EFormat mapping table. The rows must be in the exactly same order as Format enum members are defined.
    static constexpr FFormatInfo gcFormatInfo[] = {
    //        Format                   Name             Bytes BlockSize     FormatKind         red    green  blue   alpha  depth  stencil signed srgb
    ...
    FFormatInfo GetFormatInfo(EFormat Format)
    {
        ...
        if (static_cast<UINT32>(Format) > static_cast<UINT32>(EFormat::NUM))
        {
