	...
	bool SurfaceBakePass::execute(CommandListInterface* cmdlist, RenderResourceCache* cache)
	{
        ...
        if (_model_entity)
        {
            ...
            for (uint32_t ix = 0; ix < mesh->submeshes.size(); ++ix)
            {
                ...
                _binding_set_items[1] = BindingSetItem::create_texture_srv(0, check_cast<TextureInterface>(cache->require(get_geometry_texture_name(submesh.material_index, Material::TextureType_BaseColor, model_name).c_str())));
                _binding_set_items[2] = BindingSetItem::create_texture_srv(1, check_cast<TextureInterface>(cache->require(get_geometry_texture_name(submesh.material_index, Material::TextureType_Normal, model_name).c_str())));
                _binding_set_items[3] = BindingSetItem::create_texture_srv(2, check_cast<TextureInterface>(cache->require(get_geometry_texture_name(submesh.material_index, Material::TextureType_PBR, model_name).c_str())));
                _binding_set_items[4] = BindingSetItem::create_texture_srv(3, check_cast<TextureInterface>(cache->require(get_geometry_texture_name(submesh.material_index, Material::TextureType_Emissive, model_name).c_str())));
