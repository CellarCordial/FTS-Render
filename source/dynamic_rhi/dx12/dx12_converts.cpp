...
namespace fantasy
{
    ...
    D3D12_RESOURCE_STATES convert_resource_states(ResourceStates states)
    {
        ...
        if ((states & ResourceStates::PixelShaderResource) != 0) ret |= D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
        if ((states & ResourceStates::NonPixelShaderResource) != 0) ret |= D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
        ...
        if ((states & ResourceStates::CopyDest) != 0) ret |= D3D12_RESOURCE_STATE_COPY_DEST;
        if ((states & ResourceStates::CopySource) != 0) ret |= D3D12_RESOURCE_STATE_COPY_SOURCE;
        if ((states & ResourceStates::ResolveDst) != 0) ret |= D3D12_RESOURCE_STATE_RESOLVE_DEST;
        if ((states & ResourceStates::ResolveSrc) != 0) ret |= D3D12_RESOURCE_STATE_RESOLVE_SOURCE;
    ...
    DX12ViewportState convert_viewport_state(const RasterState& raster_state, const FrameBufferInfo& frame_buffer_info, const ViewportState& viewport_state)
    {
        ...
        ret.scissor_rects.resize(viewport_state.rects.size());
        for (uint32_t ix = 0; ix < ret.scissor_rects.size(); ++ix)
        {
            auto& src_scissor_rect = viewport_state.rects[ix];
    ...
    D3D12_RESOURCE_DESC convert_texture_desc(const TextureDesc& desc)
    {
        ...
        ret.SampleDesc = { desc.sample_count, desc.sample_quality };
        ret.Alignment = 0;
        ret.Height = desc.height;
        ret.Width = desc.width;
        ret.MipLevels = static_cast<uint16_t>(desc.mip_levels);
        ret.Format = desc.is_type_less ? format_mapping.resource_format : format_mapping.rtv_format;
        ...
        case TextureDimension::Texture1DArray:
            ...
            ret.DepthOrArraySize = static_cast<uint16_t>(desc.array_size);
            break;
        ...
        case TextureDimension::Texture3D:
            ...
            ret.DepthOrArraySize = static_cast<uint16_t>(desc.depth);
            break;
        case TextureDimension::Unknown:
            assert(!"invalid Enumeration value");
        }

        const FormatInfo& crFormatInfo = get_format_info(desc.format);
    ...
    D3D12_CLEAR_VALUE convert_clear_value(const TextureDesc& desc)
    {
        ...
        ret.Format = format_mapping.rtv_format;
